...
- **Trips home** — every trip you and your household have, split into
  Upcoming/Past, plus a "+ New trip" flow.
- Per trip: **Trip** dashboard, **Plan** (day-by-day itinerary), **Journey**
  (route map — curated trips only), **Costs**, and a **Bookings** list
  reached from the Trip dashboard.
- Three working sheets: **Add a cost**, **Add an activity** (with city-aware
  location suggestions on the curated trip, a plain text field otherwise),
  and **booking detail**.
...
src/
  ...
  components/
    ...
    sheets/                 ExpenseSheet, AddActivitySheet, BookingSheet
